...
int main()
{
	...
	FILE *fp=fopen("usersplit1.txt","r");
	FILE *ft=fopen("cache.txt","r");
	char buff[1024];
	...
	FILE *f=fopen("FinalFound.txt","w");//to write the all matched strings 
	int count=0;
	clock_t start, end;
	//printf("here");
	if(fp == NULL)
	...
	while((ch = fgetc(ft)) != EOF)		//scanning character by character from input file
	{
		...
		if(testMember(a1,buff1)==1&& *a1!='\n')			
			if(testMember(a2,buff2)==1&& *a2!='\n')
			    if(testMember(a3,buff3)==1&& *a3!='\n')
			         if(testMember(a4,buff4)==1&& *a6!='\n')
			              if(testMember(a5,buff5)==1&& *a5!='\n')
			                  if(testMember(a6,buff6)==1&& *a6!='\n')
			                       if(testMember(a7,buff7)==1&& *a7!='\n')
			                           if(testMember(a8,buff8)==1&& *a8!='\n')
			                             {
   ...
   /* if(count>0)
	...
	FILE *sync = fopen("FinalFound.txt", "r");
	FILE *fw=fopen("cachedb.txt","a");
	FILE *f1=fopen("userurl.txt","r");
